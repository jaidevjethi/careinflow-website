...
- Palette "Clinical Blue & Indigo", and it is **one cool family with no warm neutral anywhere**. Every neutral measures b* ≤ 0 in Lab; positive b* is the yellow axis, which is beige, and it is banned. The ground ladder is six values, not two — `surface #FFFFFF` → `canvas #EDF3F9` → `powder #DFE9F5` → `mist #D7EBE9` → mid-tones `deep #9FC4D6` / `moss #B2BCE6` → midnight `#0B2440`. Every page changes ground as you scroll; at most one mid-tone band per page. On `deep`/`moss` **`muted` and `faint` fail AA** — use `ink`, `body-deep` or `surgical`, and set `onBand` on `SectionHead`.
