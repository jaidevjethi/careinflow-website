...
- Palette "Clinical Blue & Teal", and it is **one cool family with no warm neutral anywhere**. Every neutral measures b* ≤ 0 in Lab; positive b* is the yellow axis, which is beige, and it is banned. The ground ladder is six values, not two — `surface #FFFFFF` → `canvas #EDF3F9` → `powder #DFE9F5` → `mist #D7EBE9` → mid-tones `deep #9FC4D6` / `moss #9BC7BE` → midnight `#0B2440`. Every page changes ground as you scroll; at most one mid-tone band per page. On `deep`/`moss` **`muted` and `faint` fail AA** — use `ink`, `body-deep` or `surgical`, and set `onBand` on `SectionHead`.
- Brand: `surgical #0F4C75` primary, `vivid #0FBFAE` (fills only, ink text on it, never white), `pass #0A6E62` for verified measurements ONLY, `ink #0E2439` — a deep navy at the grounds' own hue, never black. Five accents named for the service (`web`/`seo`/`gbp`/`care`/`social`), never for the hue, on a cool arc: navy, blue, teal, periwinkle, green.
