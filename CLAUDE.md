...
- **No scarcity — no founding-practice counts, no countdowns, no deadlines on a quote.** Sample interface artifacts keep their `ILLUSTRATIVE` label.
- **There is exactly one real client, and the portfolio must never imply more.** Pramukh Multispeciality Dental Clinic (Mehsana) is the only live build for a paying practice. Lavanya Skin Studio, Gati Physiotherapy and Divyam Tours are all samples with **no business behind them** — invented in full, deployed so a buyer can open them. Each carries `demo: true`, which drops the `mentions` Organization from its schema so no search engine is told the business exists, and prints the disclosure at the top of its own page. Divyam is additionally the one deliberate non-healthcare piece. Any copy that counts the projects aloud — the `/work` hero lede, the homepage band lede, `llms.txt` — says one real and three samples, and adding a project means correcting all three.
...
- **Touching that grid means re-checking `CaseStudyCard`'s cover.** The column width feeds two things that do not follow it automatically: the `sizes` attribute, which must state the real rendered box or the browser fetches a small variant and upscales it; and the cover's aspect ratio, which is `aspect-[2/1]` and must stay a *ratio* — a fixed pixel height silently becomes a letterbox the moment the column widens. Both broke on the three-to-two column change, in that order.
